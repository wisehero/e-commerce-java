...
Java 25 + Spring Boot 4 기반 이커머스 백엔드. Gradle 멀티 모듈이며, PG 시뮬레이터 앱은 Kotlin으로 구현.
...
아래 5계층 DDD 규칙은 `apps/commerce-api`와 `apps/pg-simulator` 결제 코드에 적용한다. PG 시뮬레이터도 순수 도메인 모델과 인프라 JPA 엔티티를 분리하며, `commerce-api`는 기본 `PaymentGateway` 구현으로 이 앱을 호출한다. 필요하면 설정으로 `StubPaymentGateway`를 선택할 수 있다.
...
- JPA 엔티티 연관관계 어노테이션(`@ManyToOne` 등) **전면 금지**. 상대는 ID로만 참조. 단, Aggregate 내부 값 컬렉션의 `@CollectionTable` 소유자 FK 지정은 예외.
