...
| 고객 표면 | 상품검색 brandId 필터 + `GET /api/v1/brands/{brandId}` 단건 + 상품 상세 응답 브랜드 포함 | |
| 등록 검증 | `findById`로 존재 검증 및 응답 조립, ACTIVE 불요 | |
...
- **트레이드오프(의식적 허용)**: product infra가 brand 테이블/상태 컬럼을 읽게 된다. 이는 **읽기(조회) 측** 한정이며, CQRS read-model이 테이블을 조인하는 것과 같은 성격이다. 이 조인은 다중 Aggregate 수정 예외와 무관하다. 쓰기/명령에서는 브랜드 상태 변경이 다른 Aggregate를 수정하지 않고, 다른 Aggregate를 객체 연관으로 끌어오지도 않는다. 브랜드를 도메인 객체로 끌어오는 것이 아니라 상태 컬럼만 필터한다.

### 상세 — application 2단계

- product 상세 UseCase가 `ProductRepository`로 상품을, `SkuRepository`로 SKU 목록을, `BrandRepository`로 브랜드를 **각각 조회**해 조립한다.
- 브랜드가 `INACTIVE`이면 `NOT_FOUND`(비-`ON_SALE` 상품과 동일 취급).
- 이 과정에서 읽은 브랜드의 이름/로고를 상세 응답에 그대로 쓴다(아래 §3 포함).

### 고객 브랜드 단건 조회

- `GET /api/v1/brands/{brandId}`에서 브랜드가 `INACTIVE`이면 `NOT_FOUND`. 게이트 원칙과 일관.
- 별도 운영/관리용 조회 API는 현재 없다. 필요해지면 `/api/v1/admin/brands` 하위에서 추가한다.

## 3. 상품과의 연결 · 검증

- `Product.brandId` **필수**(§8에서 product 변경). 무브랜드 상품은 존재하지 않는다.
- 상품 등록 시 `brandRepository.findById(brandId)`로 브랜드를 로드해 **존재만 검증**하고 등록 응답 조립에도 사용한다. ACTIVE는 요구하지 않는다(비활성 브랜드 출시 전 적재 허용). 존재하지 않으면 `BAD_REQUEST`.
- **브랜드 정보 포함**: 상세는 이미 2단계로 브랜드를 읽으므로 추가 비용 없이 브랜드 이름/로고를 `Info`에 담는다. 상품 상세 응답은 `brandId`만이 아니라 `brandName`·`brandLogoUrl`을 함께 보여준다. 목록 요약 응답은 현재 `id/name/imageUrl/lowestSalePrice`만 포함한다.
...
- **interfaces**: `BrandControllerV1`(`GET /api/v1/brands/{brandId}` 공개 단건 조회), `BrandAdminControllerV1`(`/api/v1/admin/brands` 관리), `*Request`/`*Response`.
...
- `ProductRegisterUseCase`에 브랜드 로드 기반 존재 검증 추가(`BrandRepository.findById`).
...
2. **product 연결** — brandId 필수화 + 등록 존재검증 + 조회 게이트 조인 + 검색 brandId 필터 + 상품 상세 응답 브랜드 포함(§8).
3. **product 문서 갱신**(§8).

## 10. 보류·확장 지점 (의식적으로 미룸)

- 브랜드 디렉토리(`GET /api/v1/brands` 전체 목록·페이징, "브랜드관").
- `description` 등 브랜드 콘텐츠 필드.
- 셀러/입점사(정산·권한 단위) — 별도 도메인.
- 인증/인가 1차 경계 적용 완료: 브랜드 운영 API는 `/api/v1/admin/**` + `ADMIN`.
