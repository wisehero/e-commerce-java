...
- `commerce-api`의 현재 결제 포트 구현은 **외부 호출 지연 스텁**(`StubPaymentGateway`, `sleep ~200ms`, 항상 성공)이다. 별도 `apps/pg-simulator` 앱은 존재하지만, 주문 흐름이 아직 그 앱을 HTTP로 호출하지는 않는다.
...
환불(PaymentGateway.refund, 스텁)   ← 트랜잭션 밖. 기존 상태가 PAID였던 주문만 호출
```
...
- `apps/pg-simulator`는 로컬 결제 검증용 별도 앱이다. 현재 `commerce-api`에는 이 앱을 호출하는 `PaymentGateway` HTTP 어댑터가 없으므로, 기본 주문 생성·취소 흐름은 `StubPaymentGateway`만 사용한다.
