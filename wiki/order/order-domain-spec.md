...
| orderedAt | (BaseJpaEntity.createdAt) | 생성 시각 |

- 팩토리: `Order.place(memberId, lines)`(id=null, status=PAYMENT_PENDING, totalAmount=라인 합) / `Order.reconstitute(...)`.
- 메서드: `markPaid()`, `cancel()`.
...
```
Txn1: memberId 존재검증
      → 각 라인 SKU·Product 조회(존재 · ON_SALE 구매가능) → 가격·이름·옵션 스냅샷
      → 재고 차감(전략 주입, §6) → Order(PAYMENT_PENDING) 저장
      → commit (SKU 락 즉시 해제)
...
- **환불 실패 처리는 보류**(스텁은 항상 성공 가정). 실제 PG라면 재시도·수동개입이 필요하지만 범위 밖.
...
- `POST /orders`의 `lockMode` 파라미터(`optimistic`/`pessimistic`/`atomic`)로 선택.
- UseCase가 `Map<String, StockDeducter>`(Spring이 빈 이름으로 주입)에서 라우팅. 한 앱을 띄워놓고 같은 부하로 3개를 번갈아 비교.
...
| UseCase | 입력 | 출력 | 비고 |
|---|---|---|---|
| 주문 생성 | `OrderPlaceCommand`(memberId, lines, lockMode) | `OrderInfo` | 2단계 흐름(§4). Txn1/외부결제/Txn2 |
