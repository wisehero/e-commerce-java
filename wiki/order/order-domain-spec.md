...
- 보류(의식적): 배송·정산 라이프사이클, WMS 재고 사가(product §10), 장바구니→주문, 멱등성, 환불 실패 처리.
...
- 팩토리: `Order.place(memberId, lines, discountAmount, usedCouponId)`(id=null, status=PAYMENT_PENDING, totalAmount=라인 합, payableAmount=총액-할인액) / `Order.reconstitute(...)`.
- 메서드: `markPaid()`, `cancel()`.
...
Txn1: 인증 memberId 존재검증
      ...
      → commit (SKU 락 즉시 해제)
결제 호출(PaymentGateway.pay, 스텁 ~200ms)   ← 트랜잭션 밖
...
- 낙관적/비관적 전략은 로드한 `Sku`에 `decreaseStock`을 적용한 뒤 그 도메인 객체의 `salePrice`·`optionSummary()`로 스냅샷을 만든다.
...
환불(PaymentGateway.refund, 스텁)   ← 트랜잭션 밖 (결제 호출과 동일 원칙)
```
...
- 현재 구현은 학습·비교 목적에 맞춰 `OrderPlaceRequest.lockMode`를 필수 입력으로 받고, `OrderPlaceUseCase`가 `Map<String, StockDeducter>`(Spring 빈 이름)에서 전략을 라우팅한다.
...
- **총액 저장**: `Order.place()`가 `Order.totalAmountOf(lines)`로 합을 **스스로 계산**해 보유 → 결제 게이트웨이에 전달 → `orders.total_amount`로 영속화.
  - 근거: 총액 = 결제에 청구한 금액 = 구체적 사실. 라인 가격을 스냅샷하는 것과 같은 논리. 조회 시 재계산 불필요.
  - 무결성: 외부에서 총액을 받지 않고 도메인이 라인 합으로 계산 → 불일치 불가능.
- **[쿠폰 확장 · 2026-06-14]** 쿠폰 도입으로 "총액 = 청구액" 등식이 깨진다. `Order`는 `totalAmount`(주문 총액=라인 합)에 더해 `discountAmount`·`payableAmount`(청구액 = 총액 − 할인)·`usedCouponId`를 보유하고, **결제·환불은 `payableAmount` 기준**이다. 청구액이 0이면 결제 게이트웨이를 건너뛰고 바로 `PAID`. `Money`에는 `minus(Money)`·정률 버림(floor)·`min(Money)`가 추가된다. 상세는 [coupon 스펙 §7·§8](../coupon/coupon-domain-spec.md).
...
| UseCase | 입력 | 출력 | 비고 |
|---|---|---|---|
| 주문 생성 | `OrderPlaceCommand`(memberId, lines, lockMode, couponId?) | `OrderInfo` | 2단계 흐름(§4). Txn1/외부결제/Txn2 |
| 주문 취소 | memberId, orderId | — | 인증 회원 본인 주문만 허용, 재고 복원 + 환불(§5) |
...
- `OrderPlaceUseCase` 의존: `MemberRepository`·`SkuRepository`·`ProductRepository`·`BrandRepository`(읽기 검증·스냅샷) + `OrderRepository`(쓰기) + `IssuedCouponRepository`(쿠폰 조회·사용·복원) + `OrderCompensationHelper`(재고·쿠폰 보상) + `StockDeducter`(포트) + `PaymentGateway`(포트).
- 라인 스냅샷에 상품명이 필요하므로 `ProductRepository`로 Product를(상품명·`ON_SALE` 구매가능 여부), `BrandRepository`로 Brand를(`ACTIVE` 구매가능 여부), `SkuRepository`로 SKU를(가격·옵션·존재) 읽어 조립한다.
